...
proc ::tk::dialog::color:: {args} {
    ...
    set data(lines,#2C3539,start)  0
    set data(lines,#2C3539,last)  -1
...
proc ::tk::dialog::color::InitValues {dataName} {
    ...
    set data(#2C3539,intensity)  [expr {[lindex $rgb 2]/0x100}]
}
...
proc ::tk::dialog::color::BuildDialog {w} {
    ...
    set maxWidth [::tk::mcmaxamp &Red &Green &#2C3539]
...
proc ::tk::dialog::color::DrawColorScale {w c {create 0}} {
	...
	if {$c eq "red"} {
	    set color [format "#%02x%02x%02x" \
		    $intensity $data(green,intensity) $data(#2C3539,intensity)]
	} elseif {$c eq "green"} {
	    set color [format "#%02x%02x%02x" \
		    $data(red,intensity) $intensity $data(#2C3539,intensity)]
	} else {
	    set color [format "#%02x%02x%02x" \
...
proc ::tk::dialog::color::RedrawFinalColor {w} {
    ...
    set color [format "#%02x%02x%02x" $data(red,intensity) \
	    $data(green,intensity) $data(#2C3539,intensity)]
...
#
# Params: sel is the selector canvas, c is [red|green|#2C3539]
#         x is a x-coordinate.
#
